...
from PyQt5.QtWidgets import QApplication, QWidget, QFileDialog, QMessageBox, QTableWidgetItem, QHBoxLayout
import soundfile as sf
import numpy as np
import TP10ultimo as tp10
...
class GUI(QWidget):
    ...
    def Process(self):
        ...
        if self.ui.mono.isChecked():
            ...
            self.results = tp10.MonoParam(self.audio, self.fs, octave, schroeder, mmf_window)
            j = 0
        ...
        if self.ui.stereo.isChecked():
            ...
            self.ui.tableWidget.setVerticalHeaderLabels(rows)
            mmf_window = int(self.ui.window.text())
            self.results = tp10.StereoParam(self.audio, self.fs, octave, schroeder, mmf_window)
            j = 0
        ...
        if self.ui.stereo_split.isChecked():
            ...
            self.ui.tableWidget.setVerticalHeaderLabels(rows)
            mmf_window = int(self.ui.window.text())
            self.results = tp10.StereoSplitParam(self.audio, self.audio3, self.fs, octave, schroeder, mmf_window)
            j = 0
            c = np.array(['Tt', 'EDTt', 'C50','C80','EDT','T20','T30', 'IACCe'])
            for i in range(0, len(c)):
                for reg in self.results[c[i]]:
